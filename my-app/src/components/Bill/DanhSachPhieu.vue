    ...
    <v-col cols="12" class="pa-0">
        <v-card>
            <v-card-text>
                ...
                <v-row>
                    <v-col cols="3">
                        <v-datepicker v-model="searchParamsBill.fromdate"
                                      @input="getDataFromApi(searchParamsBill)"
                                      hide-details
                                      label="Từ ngày"
                                      placeholder="dd/mm/yyyy"></v-datepicker>
                    </v-col>
                    <v-col cols="3">
                        <v-datepicker v-model="searchParamsBill.todate"
                                      @input="getDataFromApi(searchParamsBill)"
                                      hide-details
                                      label="Đến ngày"
                                      placeholder="dd/mm/yyyy"></v-datepicker>
                    ...
                    <v-col cols="3">
                        <v-text-field v-model="searchParamsBill.frommoney"
                                      @input="getDataFromApi(searchParamsBill)"
                                      hide-details type="number"
                                      label="Tổng tiền từ"></v-text-field>
                    </v-col>
                     <v-col cols="3">
                        <v-text-field v-model="searchParamsBill.tomoney"
                                      @input="getDataFromApi(searchParamsBill)"
                                      hide-details type="number"
                                      label="Tổng tiền đến"></v-text-field>
                    </v-col>
                    <v-col cols="12" style="padding-top:0">
                        <v-spacer></v-spacer>
                        <v-btn @click="showModalThemSua(false, {})" color="primary" style="float: right" small><v-icon small>add</v-icon> Thêm Mới</v-btn>
                    </v-col>
                </v-row>
                <v-row>
        ...
        data() {
            return {
                ...
                tableHeader: [
                  ...
                  { text: 'Ngày nhập', value: 'BillName',sortable: false, align: 'center' },
                  { text: 'Tổng tiền nhập', sortable: false, value: 'Total', align: 'center' },
